...
my %legacy_reg = (
    ...
    'vm-7000-disk-1' => {
        ...
        snapshots => {
            snap1 => { snapshot_id => '301,0', snapshot_group => 'pve_e_301_snap1', svol_ldev_id => 310, timestamp => 100 },
            snap2 => { snapshot_id => '301,1', snapshot_group => 'pve_e_301_snap2', svol_ldev_id => 311, timestamp => 200 },
        },
    },
...
subtest 'dry-run writes nothing' => sub {
    ...
    is( scalar @{ $s->{volumes} }, 3, 'summary lists the 3 committed volumes (reservation skipped)' );
...
subtest 'migrate_store: volumes, clone fields, snapshots, protected, creds' => sub {
    ...
    is( scalar @{ $s->{volumes} }, 3, '3 volumes migrated' );
...
subtest 'idempotent re-run (register merges the same backend_id)' => sub {
    my $s = PVE::Storage::FCLU::Migrate::Hitachi::migrate_store(
        storeid => $storeid, legacy_base => $legacy, fclu_base => $fclu );
    is( scalar @{ $s->{volumes} }, 3, 're-run migrates the same 3 volumes without error' );
