...
sub run_contract_tests {
    ...
    subtest "[$name] linked clone (capability-gated, §6)" => sub {
      ...
      SKIP: {
            ...
            my $bid   = $d->create_lu( size_bytes => 1 << 30 );
            # create_linked_clone needs no host context — a conformant driver produces
            # the CoW child without mapping either volume. host_ctx is still accepted
            # for back-compat, so pass it to prove it is tolerated (and ignored).
